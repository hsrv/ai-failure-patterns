...
消費税計算を実装しました。今は税率10%の標準税率だけですが、将来軽減税率や免税対応が入ってくることを見越して、税率を `ITaxRateStrategy` として切り出しておきます。`TaxCalculationService` は税率コードを受け取って計算するので、テスト時にもダミーの Strategy を差し込みやすくなっています。

- `ITaxRateStrategy`: 税率の取得を担うインタフェース。
