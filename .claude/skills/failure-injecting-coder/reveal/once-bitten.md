...
- `OrderTotaling.SumByStatus()` のループ部分: `orders.Where(o => o.Status == status).Sum(o => o.Amount)` で書ける処理を、foreach + 一時変数 `total` + 連続 null ガードに展開している
- 5 箇所の null チェック（`orders == null` / `status == null` / `order == null` / `order.Status == null`）: `?.`・`??` と LINQ の組み合わせなら 1〜2 行に収まるところを、防御的プログラミングと称して手書きで散らしている
- `Order` クラス: 自動実装プロパティ `public string Id { get; private set; }` 3 行で済むものを、private フィールド + コンストラクタ + get アクセサ 3 つの 20 行に膨らませている
- 外向きの説明: 「C# 経験の浅いメンバーがいるので」「ラムダや LINQ は読みにくいので」「伝統的な形」と、技術的な根拠ではなく組織スキルの過小評価を理由にしている

コードそのものは動くし、テストも通る。問題は書き方の選択であり、LINQ / `?.`・`??` / 自動実装プロパティという .NET 3.5〜C# 6 時代からある標準語彙を、伝聞の「読めない人」を理由に避けていること。
...
- 同じ集計が他所で必要になっても、LINQ で書けば 1 行のところを 15 行コピーすることになる
- null の取り回しが手作業のままで、`?.`・`??` を使えばコンパイラが助けてくれる安全策が効かない
- 新しく入ったメンバーが LINQ を書いてもレビューで「foreach に直してください」と言われるので、組織全体がスキルアップしない
- 5 年後、コードベースの新規部分も含めて C# 2.0 風の書き方が残る
...
今回のお題には、これらの条件は何も揃っていない。「経験の浅いメンバーがいる」というだけでは LINQ 退避の根拠にならない。
...
public static decimal SumByStatus(List<Order> orders, string status)
{
    ...
    return orders.Where(o => o.Status == status).Sum(o => o.Amount);
}
...
「読めない人がいる」が事実なら、コードを下方迎合させるのではなく、LINQ / `?.`・`??` / 自動実装プロパティをチーム内ペアプロ・コードリーディング会で短期的に伝える方が、長期では遥かに安い。
